...
def test_no_star_appears_on_both_sides_of_a_fold(dataset, n_splits):
    ...
    splits = grouped_splits(y, dataset.star_id, n_splits, seed=42)
...
def test_every_row_is_validated_exactly_once(dataset):
    y = dataset.require_supervision()
    splits = grouped_splits(y, dataset.star_id, 3, seed=42)
...
def test_nested_inner_splits_stay_inside_the_outer_training_stars(dataset):
    y = dataset.require_supervision()
    for outer_train, outer_val in grouped_splits(y, dataset.star_id, 3, seed=42):
        held_out = set(dataset.star_id[outer_val])
        inner = grouped_splits(
            y[outer_train], dataset.star_id[outer_train], 2, seed=42
        )
...
def test_splits_are_reproducible(dataset):
    y = dataset.require_supervision()
    first = grouped_splits(y, dataset.star_id, 3, seed=42)
    second = grouped_splits(y, dataset.star_id, 3, seed=42)
...
def test_full_fold_count_is_kept_when_support_allows(dataset):
    y = dataset.require_supervision()
    assert resolve_n_splits(y, dataset.star_id, 5, 3, "outer") == 5
...
def test_insufficient_diversity_raises_a_diagnostic():
    ...
    assert "distinct stars per label" in message
...
# ---------------------------------------------------------------------------
def test_every_row_trains_bases_and_the_meta_model(dataset, fast_config, monkeypatch):
    """Base learners and the meta-model both see every candidate, no status gate."""
    y = dataset.require_supervision()
    ...
    fit_stack(dataset.X, y, dataset.star_id, fast_config, dataset.feature_names)
    ...
    # The final meta fit uses every row too.
    assert meta_fit_labels, "the meta-model must be fitted"
    assert n_total in [len(labels) for labels in meta_fit_labels]

    # Thresholds are chosen over the full cross-fitted set.
    assert threshold_labels


def test_threshold_is_derived_from_cross_fitted_predictions(dataset, fast_config):
    y = dataset.require_supervision()
    result = fit_stack(dataset.X, y, dataset.star_id, fast_config, dataset.feature_names)
    crossfit = result.meta_crossfit
    assert not np.isnan(crossfit).all()
...
def test_meta_inputs_exclude_validation_stars_from_all_upstream_fits(
    ...
    tr, va = grouped_splits(y, groups, 2, seed=42)[0]
    ...
    def trace_fit(learner, params, X_, y_, groups_, config):
        stars = set(groups_)
        assert stars.isdisjoint(held_out)
        fit_stars.append(stars)
        fitted = real_fit_base(learner, params, X_, y_, groups_, config)
    ...
    def trace_tune(learner, X_, y_, groups_, splits, config, progress=None):
        assert set(groups_).isdisjoint(held_out)
        tuning_stars.append(set(groups_))
        return real_tune_base(learner, X_, y_, groups_, splits, config, progress)

    monkeypatch.setattr(stacking, "fit_base", trace_fit)
    monkeypatch.setattr(stacking, "tune_base_learner", trace_tune)
    folds = stacking.prepare_meta_folds(X, y, groups, [(tr, va)], fast_config)
    fold = folds[0]
    ...
    def trace_meta(P, y_, groups_, config, params=None):
        np.testing.assert_array_equal(P, fold.train_probabilities)
        np.testing.assert_array_equal(y_, y[tr])
        np.testing.assert_array_equal(groups_, groups[tr])
        meta_calls.append(params)
        fitted = real_fit_meta(P, y_, groups_, config, params)
        real_predict = fitted.predict_proba

        def trace_predict(P_val):
            np.testing.assert_array_equal(P_val, fold.val_probabilities)
    ...
    tuning = stacking.tune_meta_learner(folds, y, groups, fast_config)
    predictions = stacking.crossfit_meta_predictions(
        folds, y, groups, fast_config, tuning.best_params
    )
...
def test_meta_training_inputs_are_invariant_to_held_out_data(
    ...
    y = dataset.require_supervision()
    groups = dataset.star_id
    split = grouped_splits(y, groups, 2, seed=42)[0]
    _, va = split
    original = stacking.prepare_meta_folds(dataset.X, y, groups, [split], fast_config)[0]
    ...
    changed = stacking.prepare_meta_folds(
        changed_X, changed_y, groups, [split], fast_config
    )[0]
...
def test_meta_input_isolation_reports_insufficient_nested_star_support(fast_config):
    ...
    with pytest.raises(DataDiversityError, match=r"base-within-meta.*meta fold 1"):
        stacking.prepare_meta_folds(X, y, groups, splits, fast_config)
